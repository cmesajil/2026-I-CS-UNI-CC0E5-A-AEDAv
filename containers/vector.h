...
public:
  using general_iterator<Container,
                         vector_forward_iterator<Container>>::general_iterator;
...
public:
  using general_iterator<Container,
                         vector_backward_iterator<Container>>::general_iterator;
  vector_backward_iterator operator++() {
    this->m_pNode--;
    return *this;
  }
};
...
public:
  ...
  backward_iterator rbegin() {
    return backward_iterator(this, m_data + m_size - 1);
  }
...
template <typename T> Vector<T>::~Vector() { delete[] m_data; }
...
template <typename T> void Vector<T>::push_back(value_type value, Ref ref) {
...
template <typename T>
typename Vector<T>::value_type Vector<T>::get(size_t index) {
