...
public:
    ...
    friend ostream& operator<<(ostream& os, const LLNode<T>& node){
        return os << "(" << node.getData() << ", " << node.getDataRef() << ")";
    }
};
...
public:
    ...
    LinkedList(const LinkedList &other): m_pRoot(nullptr), m_tail(nullptr), m_size(0){
        ...
        while (current != nullptr) {
            // Reutilizamos tu push_back para replicar los elementos
            // Asegúrate de que push_back sea seguro y no fuerce un orden que rompa la copia
            this->push_back(current->getData(), current->getDataRef());
            current = current->getNext();
        }
    }

    // DONE: Constructor de Movimiento
    LinkedList(LinkedList&& other) noexcept : m_pRoot(nullptr), m_tail(nullptr), m_size(0) {
